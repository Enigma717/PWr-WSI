...
function check_forwinning(gameboard::Gameboard{Int8}, player::Integer)
    ...
    ###  Check condition in all columns  ###
    ...
    ###  Check condition in all diagonals  ###
...
#=
    ...
    Direct blocks are evaluated higher than indirect ones.
=#
function check_blocks(gameboard::Gameboard{Int8}, player::Integer)
    eval::Int32 = NEUTRAL_EVAL
    diagonals::Vector{Vector{Int8}} = getdiagonals(gameboard)


    ###  Check blocks in all rows  ###
    for i in 1:gameboard.size
        row::Vector{Int8} = gameboard.board[i, :]
...
#=
    ...
    If the game is after first 4 moves (2 from each player), function checks:
    ...
    Being aggresive is evaluated higher than being defensive, but nevertheless
    blocking in dangerous situations still has highest priority. 


    NOTE: From manual testing it seems that this algorithm performs better 
    if it plays as the second player. Probably because it forces situations
    where opponent has to place three consecutive symbols, where the AI itself
    still has options to win by placing four. 
    ...
    possibility of pointless evaluating positions that would finish the game.
=#
function heuristiceval(gameboard::Gameboard{Int8}, depth::Integer, player::Integer)
    evaluation::Int32 = NEUTRAL_EVAL


    ###  If the game is after 4 moves  ###
    if gameboard.movesdone > 4
    ...
    ###  If the game is after 3 moves  ###
    if gameboard.movesdone > 3
        ###  Calculate and add blocks from the player  ###
